...
public class AnimationCard extends Table {
    public AnimationCard(String pamPath) {
        ...
        String name = PamUtils.getPamNameFromPath(pamPath);
        String categoryName = PamUtils.getPamCategoryFromPath(pamPath);

        Label titleLabel = new Label(PamUtils.prettify(name), skin, "medium");
        titleLabel.setColor(Color.BLACK);
        Label category = new Label(PamUtils.prettify(categoryName), skin, "secondary");

        titleLabel.setEllipsis(true);
        category.setEllipsis(true);

        Table left = new Table();
        left.add(titleLabel).growX().minWidth(0).row();
        left.add(category).growX().minWidth(0);

        PamActor pamDrawable = new PamActor(pamPath);
        pamDrawable.setFitToBounds(true);

        internal.add(left).grow();
        internal.add(pamDrawable).growY().width(60);
